...
class DesignJobsScreen extends StatelessWidget {
  const DesignJobsScreen({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    // Filter design related jobs
    final designJobs = controller.allJobs.where((job) => 
      job.title.toLowerCase().contains('design') || 
      job.title.toLowerCase().contains('creative') ||
    ...
    return Scaffold(
      ...
      body: designJobs.isEmpty 
        ? const Center(child: Text("No design jobs available at the moment."))
        : ListView.builder(
            padding: const EdgeInsets.all(16),
            itemCount: designJobs.length,
            itemBuilder: (context, index) {
              final job = designJobs[index];
        ...
        child: Row(
          children: [
            Container(
              ...
              child: Icon(Icons.design_services, color: job.themeColor),
            ),
